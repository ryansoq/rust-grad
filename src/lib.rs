...
mod tests {
    ...
    fn test_micrograd_example() {
        ...
        let c = &c + &(&c + &Value::new(1.0));      // c = -2 + (-2+1) = -3
        let c = &c + &(&(&Value::new(1.0) + &c) + &(-&a)); // c = -3 + (1+(-3)) + 4 = -1
        let d2 = &d * &Value::new(2.0);
        let ba_relu = (&b + &a).relu();
        let d = &d + &(&d2 + &ba_relu);                // d = 0 + (0 + relu(-2)) = 0
        let d3 = &Value::new(3.0) * &d;
        let bna_relu = (&b + &(-&a)).relu();
        let d = &d + &(&d3 + &bna_relu);               // d = 0 + (0 + relu(6)) = 6
        let e = &c + &(-&d);                        // e = -1 - 6 = -7
        let f = e.pow(2.0);                         // f = 49
        // 簡化版：只驗到 e（還沒實作 div）
        assert!((e.data() - (-7.0)).abs() < 1e-6);
        e.backward();
        // 驗證梯度不是 NaN
        assert!(!a.grad().is_nan());
        assert!(!b.grad().is_nan());
    }
}
